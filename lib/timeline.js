...
class Timeline {
    ...
    draw(now, timeToLimit, lifetimeDrinks) {
        ...
        for (const lifetimeDrink of lifetimeDrinks) {
            if (lifetimeDrink.startedAt === lifetimeDrink.endsAt) {
                continue;
            }
        ...
        for (const lifetimeDrink of lifetimeDrinks) {
            if (lifetimeDrink.virtualStartedAt === lifetimeDrink.virtualEndsAt) {
                continue;
            }
    ...
    _computeTimelineSize(lifetimeDrinks) {
        ...
        for (const lifetimeDrink of lifetimeDrinks) {
            ...
            if (lifetimeDrink.startedAt < lifetimeDrink.endsAt) {
                effectiveDrinkCount += 1;
            }
        }

        let width = (endTime - startTime) * TIME_RATIO + (MARGINS * 2);
        const height = (effectiveDrinkCount * 2 + 1) * (DRINK_HEIGHT + 1) + (MARGINS * 2) - 1;
    ...
    _drawTime(dt, className) {
        ...
        if (x >= timelineElement.clientWidth - 1) {
            return;
        }
