...
export const computeBloodAlcoholConcentration = function(drinks, now, options) {
    ...
    for (const drinkObject of sortedDrinks) {
        ...
        drinkObject.drink.endsAt = drinkObject.drink.startedAt + (timeToZero * MILLISECONDS_PER_HOUR);

        lifetimeDrinks.push(new LifetimeDrink(bloodAlcoholConcentration, drinkObject.drink.startedAt, drinkObject.drink.endsAt, drinkObject.index));
    }
