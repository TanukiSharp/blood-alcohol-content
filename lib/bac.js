...
export class LifetimeDrink {
    ...
    constructor(bloodAlcoholConcentration, startedAt, endsAt, index) {
        ...
        this.STARTED_AT_FOR_HUMANS = new Date(startedAt);
        this.ENDED_AT_FOR_HUMANS = new Date(endsAt);
    }
}
...
const lineupLifetimeDrinks = function(lifetimeDrinks) {
    ...
    for (let i = 1; i < lifetimeDrinks.length; i += 1) {
        if (lifetimeDrinks[i].startedAt < lifetimeDrinks[i - 1].endsAt) {
            ...
            lifetimeDrinks[i].endsAt = lifetimeDrinks[i].virtualStartedAt + duration;
...
const setEffectiveDrinks = function(now, lifetimeDrinks) {
    ...
    for (let i = 0; i < lifetimeDrinks.length; i += 1) {
        if (lifetimeDrinks[i].virtualStartedAt <= now && now <= lifetimeDrinks[i].endsAt) {
...
export const computeBloodAlcoholConcentration = function(drinks, now, options) {
    ...
    debug.clear();
    debug.startDrawDrinks(sortedDrinks[0].drink.startedAt);
    ...
    for (const drinkObject of sortedDrinks) {
        ...
        debug.drawDrink(drinkObject.drink.startedAt, endsAt, drinkObject.index.toString());
    }
    ...
    debug.startDrawDrinks(lifetimeDrinks[0].startedAt);

    for (const lifetimeDrink of lifetimeDrinks) {
        debug.drawDrink(lifetimeDrink.virtualStartedAt, lifetimeDrink.endsAt, lifetimeDrink.index.toString(), lifetimeDrink.isEffective ? 'red' : undefined);
    }

    debug.drawTime(now, 'red');
    ...
    console.log(`firstEffectiveDrinkTime: ${new Date(firstEffectiveDrinkTime)}`);
