...
const computeAlcoholMass = function(drinkQuantity, alcoholPercentage) {
    return Math.max(0, drinkQuantity) * (Math.max(0, alcoholPercentage) / 100.0) * ALCOHOL_VOLUMIC_MASS * 1000;
};
...
export class LifetimeDrink {
     ...
     * @param {boolean} isValid Whether the drink has correct parameters and is not in the future.
     * @param {number} index The index of the original drink.
     */
    constructor(bloodAlcoholConcentration, startedAt, endsAt, isValid, index) {
...
const lineupLifetimeDrinks = function(lifetimeDrinks) {
    ...
    for (let i = 1; i < lifetimeDrinks.length; i += 1) {
        if (lifetimeDrinks[i].virtualStartedAt < lifetimeDrinks[i - 1].virtualEndsAt) {
            const duration = lifetimeDrinks[i].endsAt - lifetimeDrinks[i].startedAt;
            lifetimeDrinks[i].virtualStartedAt = lifetimeDrinks[i - 1].virtualEndsAt;
...
const isValidDrink = function(now, drink) {
    if (drink.quantity <= 0 || drink.alcoholPercentage <= 0) {
        return false;
    }

    const startedAt = new Date(drink.startedAt).getTime();
...
export const computeBloodAlcoholConcentration = function(drinks, now, options) {
    ...
    for (const drinkObject of sortedDrinks) {
        const isValid = isValidDrink(now, drinkObject.drink);
        ...
        lifetimeDrinks.push(new LifetimeDrink(bloodAlcoholConcentration, drinkObject.drink.startedAt, endsAt, isValid, drinkObject.index));
    }
